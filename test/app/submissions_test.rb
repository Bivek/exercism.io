...
class SubmissionsTest < Minitest::Test
  ...
  def test_disable_opinions
    ...
    Message.stub(:ship, nil) do
      login(alice)
      post "/submissions/#{submission.key}/opinions/disable"
    end
  ...
  def test_like_a_submission
    ...
    login(bob)
    post "/submissions/#{submission.key}/like"
  end
  ...
  def test_unlike_a_submission
    ...
    login(bob)
    post "/submissions/#{submission.key}/unlike"
  end
  ...
  def test_mute_submission
    ...
    Message.stub(:ship, nil) do
      login(alice)
      post "/submissions/#{submission.key}/mute"
    end
  ...
  def test_unmute_submission
    ...
    Message.stub(:ship, nil) do
      login(alice)
      post "/submissions/#{submission.key}/unmute"
    end
  ...
  def test_edit_comment
    ...
    login(bob)
    post "/submissions/#{submission.key}/nits/#{comment.id}", {body: "OK"}
  ...
  def test_delete_comment
    ...
    login(bob)
    delete "/submissions/#{submission.key}/nits/#{comment.id}"
    assert_equal 0, Comment.count
  end
end
